# Halium-Style GSI Architecture (AIDL Variant)
...
|---|------------|-------------|
| 1 | **`boot.img` must remain the stock OEM image** | Custom kernels and custom ramdisks are forbidden. PID 1 is therefore the OEM Android init. |
...
| 3 | **Do not flash `boot`, `vendor_boot`, `dtbo`, or `vendor`** | Flash `system` + disabled `vbmeta*`. Flash `userdata` on first install to seed `/data/ubuntu-gsi/rootfs.erofs` (skip for everyday system updates). |
...
* Android boots normally, all the way through `init.rc`, `zygote`, and HAL
  service registration (AIDL services on `/dev/binder` plus the small set of
  legacy HIDL services on `/dev/hwbinder`).
* A late-boot `service` declared in `/system/etc/init/ubuntu-gsi.rc` launches
  `/system/bin/ubuntu-gsi-launcher`.
* The launcher mounts a self-contained Ubuntu rootfs (`erofs`) from
  `/data/ubuntu-gsi/rootfs.erofs` (seeded by `userdata.img`, with optional heal
  from a system-partition copy), sets up bind-mounts, and `chroot`s `systemd`
  into it.
...
* HAL access (audio, camera, sensors, telephony, GNSS, vibrator, fingerprint,
  bluetooth, wifi, …) is performed **directly via `/dev/binder`** because
  Android already started `servicemanager` and every vendor HAL service. No
  custom bridge process is required.
...
sequenceDiagram
    ...
    AI->>AI: Start servicemanager, vendor HAL services, zygote, surfaceflinger…
    ...
    L->>L: ensure /data/ubuntu-gsi/rootfs.erofs (heal from system seed if needed)
    L->>L: erofs-mount /data/ubuntu-gsi/rootfs.erofs → /halium/base
...
| Path | Origin | Purpose |
|------|--------|---------|
| `/system/bin/init`, `/system/etc/init/*.rc`, `/system/lib*/`, `/system/apex/`, `/system/framework/` | **PHH Treble GSI v559+ (Android 14)** | Provides a fully working Android base because the device's vendor partition is AIDL/Android-12+. |
...
| `/system/usr/share/ubuntu-gsi/rootfs.erofs` | **This repo, built from `debootstrap`** | Optional in-system seed (`ROOTFS_SEED_IN_SYSTEM=1`) used to heal `/data/ubuntu-gsi/rootfs.erofs`. Primary runtime image lives on `/data`. |
...
`/dev/binder`, `/dev/hwbinder`, `/dev/vndbinder` appear automatically through
`/dev/binderfs`. **No custom bridge daemon is needed.**
...
The solution is to flash a **disabled** vbmeta image (`builder/out/vbmeta-disabled.img`)
with `flags=3` baked in at build time. Prefer the Makefile / flash helper:

```bash
make flash-vbmeta
# or flash vbmeta_a / vbmeta_system_a / vbmeta_vendor_a as on your device
```

Do **not** pass `--disable-verity` / `--disable-verification` to fastboot when
flashing the standalone image (fails on fastboot 34+ with `AVB_MAGIC` errors).

The image is generated by `scripts/build_vbmeta_disabled.sh` (avbtool or a
handcrafted header) with flags 1 = `HASHTREE_DISABLED`, flags 2 =
`VERIFICATION_DISABLED` (combined **3** on MTK dynamic-partition devices).

Related runtime docs: `docs/flash_quickstart.md`, `docs/lower-layer-display.md`,
`docs/hal-bridge-matrix.md`.
...
* `binder/binder-bridge.sh` (AIDL) / `hwbinder/hwbinder-bridge.sh` (HIDL)
  — Android already runs `servicemanager` / `hwservicemanager`.
* `aidl/*/hal.sh` and `hidl/*/hal.sh` HAL wrappers
  — vendor HAL services are reachable directly via `/dev/binder`.
